import { cn } from "@personalWebsite/ui/lib/utils";
...
function Card({
	...
	return (
		<div
			...
			data-size={size}
			className={cn(
				"group/card flex flex-col gap-4 overflow-hidden rounded-none border-2 border-border bg-card py-4 text-card-foreground text-xs/relaxed has-[>img:first-child]:pt-0 has-data-[slot=card-footer]:pb-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none",
				className,
			)}
			{...props}
		/>
